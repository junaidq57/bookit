...
trait EntrustUserTrait
{
    ...
    {   //soft delete undo's
        ...
        if(Cache::getStore() instanceof TaggableStore) {
//            Cache::tags(Config::get('entrust.role_user_table'))->flush();
            Cache::tags(Config::get('entrust.user_company_table'))->flush();
        }
        return $result;
    }

    public function restoreWorker()
    {   //soft delete undo's
        $result = parent::restore();
        if(Cache::getStore() instanceof TaggableStore) {
            Cache::tags(Config::get('entrust.user_company_table'))->flush();
    ...
    {
        return $this->belongsToMany(Config::get('entrust.companies'), Config::get('entrust.=user_company_table'), Config::get('entrust.user_foreign_key'), Config::get('entrust.company_foreign_key'));
    }
