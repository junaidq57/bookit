...
class CheckAdminPermission
{
    ...
    {
        $routeName = str_replace([$request->route()->action['prefix'] . '.', 'store', 'update'], ['', 'create', 'edit','clock','admin.clock','users.add','users.createWorker'], $request->route()->action['as']);
        ...
        if (\Auth::guest()) {
            ...
            return $next($request);
        } else if (\Auth::user() && in_array($routeName, ['logout', 'clock', 'users.add', 'users.createWorker'])) {
